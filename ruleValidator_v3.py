...
def saveReport(name, df, file_out):
    df.to_csv(file_out + name)


def matchLHS(data, dfReport, sizeLHS, lhsAux, sizeRHS, rhsAux, k, file_out):
            ...
            if i in listData:
                ...
                if count == sizeLHS:
                    if matchRHS(listData, sizeRHS, rhsAux):
                        ...
                        saveReport('ruleValidator.csv', dfReport, file_out)
                    else:
...
if __name__ == '__main__':
    ...
    def main(file_in, file_rules, file_out):
        ...
        for index, row in df_rules.iterrows():
            ...
            if df_rules['LHS_size'].at[index] >= 1:
                ...
                #
                # print(index)
                matchLHS(data, dfReport, sizeLHS, lhsAux, sizeRHS, rhsAux, index, file_out)
                # if MatchLHS:
